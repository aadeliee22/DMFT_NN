...
int main()
{
	...
	//std::string UU[] = {"1.50", "1.80", "2.00", "2.20", "2.25", "2.30", "2.40", "2.50","2.55", "2.60", "2.80", "3.00", "3.20", "3.50"};
	std::string UU[] = {"1.50", "2.00", "2.10", "2.20", "2.30", "2.40", "2.50","2.60", "2.70", "2.80", "3.00", "3.50"};
	
	for (int j = 0; j < 12; j++){
	
		double U = std::atof(UU[j].c_str());
		double mu = U/2.0;
		M[0] = -0.5*U, M[1] = 0.0, M[2] = 0.0;
		//std::ifstream filein1; filein1.open("./Giw/Giw_1to4_1000_" + UU[j] + ".dat");
		//std::ifstream filein2; filein2.open("./Giw/Giw_4to1_1000_" + UU[j] + ".dat");
		std::ifstream filein1; filein1.open("../../1_dmftsolver/IPT/DMFT_IPT_SOLVER/1to4_0.02/Giw-" + UU[j] + ".dat");
		std::ifstream filein2; filein2.open("../../1_dmftsolver/IPT/DMFT_IPT_SOLVER/4to1_0.02/Giw-" + UU[j] + ".dat");
		if (filein1.is_open()){
			std::cout << std::fixed;
