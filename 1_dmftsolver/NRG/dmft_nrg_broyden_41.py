...
parser.add_argument("-T", help = "temperature", type = float, default = 1e-2)
...
tol = args.tol
lamb = args.lamb
filename = f'./B_41_beta100/Bethe-{U:.3f}.dat'
#==============================================
...
sp = { "T": T, "Lambda": lamb, "Nz": 4, "Tmin": 1e-6, \
        "keep": 10000, "keepmin":1000, "keepenergy": keepE, "alpha": 0.4, "bandrescale": 1.0 }
# Model Parameters
mp = { "U1": U, "eps1": e_f }
...
{lamb:.1f}
    ...
    lattice_solver.update_hybridization_function(imp_solver, imp_solver.Delta_w['imp'].data)
...
def solve_mix_B(Delta, alpha):
    ...
    verbose=True, reduction_method="svd", max_rank = 10, f_tol = tol, maxiter = niter)
