...
parser.add_argument("-Occ", help = "Occupancy goal", type = float, default = 0.5)
...
parser.add_argument("-way", help = "direction", type = int, default = None)
...
lamb = args.lamb
way = args.way
filename = f'./BD_{Occ:.2f}_{way:.0f}/Bethe-{U:.3f}.dat'
#==============================================
# particle-hole non-symmetric case
mu = U * Occ
B = 0.0
...
def dmft_step(Delta_in):
    ...
    lattice_solver.update_hybridization_function(imp_solver, (D/2)**2 * G_new['imp'].data) # self-consistency
    ...
    if mpi.is_master_node():
        np.savetxt(filename, np.array([realFreq.real, \
            ...
            header = f"w   G_w   Sigma_w    chi_NN_w    chi_SS_w  (D: {D}, T: {T}, U: {U}, Occ: {Occ}, <n>: {nexpv:.7f})")
