...
lamb = args.lamb
way = args.way
filename = f'./C_{way:.0f}_beta10000/Cubic-{U:.3f}.dat'
#==============================================
...
lattice_solver = utility_cubic.BetheLatticeSolver(imp_solver, D)
# Load previous data
...
def ht1(z):
    ...
    dos = 1/z * np.sqrt(1-3/4*x1) / (1-x1) * ((2/np.pi)*mpm.ellipk(complex(kp2))) * ((2/np.pi)*mpm.ellipk(complex(km2)))
    return dos
...
def dmft_step(Delta_in):
    ...
    G_new, Delta_out = calc_G(imp_solver.Sigma_w, mu)
    lattice_solver.update_hybridization_function(imp_solver, Delta_out['imp'].data)
    ...
    if mpi.is_master_node():
        np.savetxt(filename, np.array([realFreq.real, \
            ...
            imp_solver.Delta_w['imp'].data.reshape([-1]).real, \
            imp_solver.Delta_w['imp'].data.reshape([-1]).imag, \
            ...
            header = f"w    A_w   G_w   Delta_w   Sigma_w    chi_NN_w    chi_SS_w  (D: {D}, T: {T}, U: {U}, half-filling)")
